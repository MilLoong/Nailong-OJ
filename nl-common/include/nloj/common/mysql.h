...
/**
 * @brief 拼进引号前转义，防止 SQL 注入。缓冲区按 2*n+1 开。
 */
std::string escape_sql(MYSQL* conn, const std::string& raw);

/**
 * @brief 执行 SELECT。
 * @return 失败返回 nullptr，成功后调用方 mysql_free_result
 */
MYSQL_RES* query_select(MYSQL* conn, const std::string& sql);

/**
 * @brief 执行 INSERT / UPDATE，没有结果集。
 * @return 成功 1，失败 0
 */
bool query_exec(MYSQL* conn, const std::string& sql);

/**
 * @brief 预处理：sql 里用 ? 绑定字符串。
 * @param rows SELECT 时填入；nullptr 表示不取结果集
 * @param insert_id INSERT 时可拿 last insert id
 * @return 成功 1，失败 0
 */
...
/**
 * @brief 独立短连接（测试夹具）。失败时已 close，调用方不要再 close。
 */
